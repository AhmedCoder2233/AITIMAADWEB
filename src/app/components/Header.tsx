...
export default function Header() {
  ...
  const navItems = [
    ...
    { href: '/join', label: 'Join Us', icon: User },
          ...
          <nav className="hidden md:flex items-center space-x-1 lg:space-x-2">
            {navItems.map((item) => {
              ...
              return (
                ...
                >
                  <span className={`relative z-10 flex items-center gap-2 ${isActive ? 'text-green-600' : 'text-gray-600 hover:text-gray-900'}`}>
                    <Icon className="h-4 w-4" />
                    <span>{item.label}</span>
                  </span>
                  <span className={`absolute inset-x-0 bottom-0 h-0.5 bg-green-600 rounded-full ${isActive ? '' : 'scale-x-0 group-hover:scale-x-100'} transition-transform origin-left`}></span>
                  <span className={`absolute inset-0 rounded-lg ${isActive ? 'bg-green-50 opacity-100' : 'bg-gray-50 opacity-0 group-hover:opacity-100'} transition-opacity`}></span>
                </Link>
              );
            })}
                  ...
                  {navItems.map((item) => {
                    ...
                    return (
                      <Link
                        key={item.href}
                        href={item.href}
                        className={`flex items-center gap-3 py-2.5 px-4 text-sm font-medium rounded-lg transition-colors ${isActive ? 'bg-green-50 text-green-700' : 'text-gray-700 hover:bg-gray-50'}`}
