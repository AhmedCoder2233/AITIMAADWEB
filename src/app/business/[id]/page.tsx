...
export default function BusinessPage() {
  ...
  // FIXED: Remove automatic migration from here - migration will happen in home page
  // useEffect(() => {
  //   const checkAndMigrateDrafts = async () => {
  //     if (user && business?.id) {
  //       await migrateLocalDraftToDatabase();
  //     }
  //   };
    
  //   checkAndMigrateDrafts();
  // }, [user, business?.id]);
  ...
  const checkForDraftReview = async () => {
    ...
    try {
      if (user) {
        // Check database for logged in user's draft
        const { data, error } = await supabase
          .from('draft_reviews')
          .select('*')
          .eq('business_id', params.id)
        ...
        if (data) {
          ...
          // FIX: Check both proof_url and proof_file_data for preview
          if (data.proof_url) {
            setProofPreview(data.proof_url);
          } else if (data.proof_file_data) {
            setProofPreview(data.proof_file_data);
          }
        } else {
          // Check if user has any local draft that needs migration
          const localDraft = getLocalDraft();
          if (localDraft) {
            // If user is logged in but has local draft, migrate it
            await migrateLocalDraftToDatabase();
          }
        }
      } else {
        ...
        if (localDraft) {
          ...
          // FIX: Set proof preview from localStorage
  ...
  const getLocalDraft = (): DraftReview | null => {
    ...
    if (savedDraft) {
      try {
        ...
        if (draft.proof_file_data && !draft.proof_file_data.startsWith('data:')) {
  ...
  const handleAddReview = () => {
    if (business?.verification_status !== 'verified') {
      showNotification('This business is not verified yet. Reviews are not available.', 'warning');
      return;
    }
                      ...
                      <form onSubmit={handleSubmitReview} className="space-y-6">
                        ...
                        <div>
                          <label className="block text-sm font-medium text-gray-700 mb-2">
                            Upload Proof {!user && '(Will be saved locally)'}
                          </label>
                        ...
                        <div className="flex gap-3 flex-wrap">
                          {user ? (
                            <>
                              <button
                                type="submit"
                                disabled={submitting || !profile?.is_verified}
                              ...
                              >
                                {submitting ? 'Publishing...' : profile?.is_verified ? 'Publish Review' : 'Verify to Publish'}
                              </button>
                              {/* Logged in users cannot save drafts, only publish */}
                            </>
                          ) : (
                            <>
                              <button
                          ...
                          <div className="flex items-start gap-4">
                            ...
                            <div className="flex-1">
                              ...
                              <div className="flex items-center gap-4">
                                <button
                                  onClick={() => handleLikeComment(comment.id, comment.likes, comment.user_liked || false)}
                                ...
                                >
                                  <ThumbsUp className={`h-4 w-4 ${comment.user_liked ? 'fill-current' : ''}`} />
