...
import { setCookie, getCookie, deleteCookie, removeDraftId, trackDraftId } from '../../lib/cookieUtils';
import { 
...
export default function BusinessPage() {
  ...
  const getDeviceId = async (): Promise<string> => {
    const savedDeviceId = await getCookie('device_id');
    ...
    await setCookie('device_id', deviceId, 30);
...
const getLocalDraft = async (): Promise<DraftReview | null> => {
  if (!business?.id) {
    return null;
  }
  
  const cookieName = `draft_review_${business.id}`;
  
  try {
    const savedDraft = await getCookie(cookieName);
    
    if (!savedDraft) {
      return null;
    }
    
    // If there's large data stored in localStorage, retrieve it
    if (savedDraft.large_data_id) {
      const largeData = localStorage.getItem(savedDraft.large_data_id);
      if (largeData) {
        savedDraft.proof_file_data = largeData;
        
        // Delete from localStorage after retrieving
        localStorage.removeItem(savedDraft.large_data_id);
      }
      savedDraft.large_data_id = undefined;
      savedDraft.has_large_proof = undefined;
    }
    
    // Ensure proof_file_data is properly set
    if (savedDraft.proof_file_data && typeof savedDraft.proof_file_data === 'string') {
      if (!savedDraft.proof_file_data.startsWith('data:')) {
        if (savedDraft.proof_file_type?.startsWith('image/')) {
          savedDraft.proof_file_data = `data:${savedDraft.proof_file_type};base64,${savedDraft.proof_file_data}`;
        } else {
          savedDraft.proof_file_data = `data:image/jpeg;base64,${savedDraft.proof_file_data}`;
        }
      }
    }
    ...
    console.error('Error parsing saved draft from cookie:', error);
...
const saveToLocalStorage = async (draftData: Partial<DraftReview>): Promise<DraftReview | null> => {
  if (!business?.id) return null;
  
  const deviceId = await getDeviceId();
  const cookieName = `draft_review_${business.id}`;
  
  // Handle large proof data separately
  let largeDataId = null;
  let draftToSave: any = {
    ...draftData,
    device_id: deviceId,
  ...
  // If proof file data is large, store it separately in localStorage
  if (draftData.proof_file_data && draftData.proof_file_data.length > 1000) {
    largeDataId = `large_data_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    
    // Store large data in localStorage
    localStorage.setItem(largeDataId, draftData.proof_file_data);
    
    // Store only metadata in cookie
    draftToSave.large_data_id = largeDataId;
    draftToSave.proof_file_data = undefined; // Remove from cookie
    draftToSave.has_large_proof = true;
  }
  
  const success = await setCookie(cookieName, draftToSave, 7);
  
  if (success) {
    // Track this draft ID
    await trackDraftId(business.id);
    return draftToSave as DraftReview;
  }
...
const deleteDraftReview = async () => {
  if (!draftReview || !business?.id) return;
  ...
  try {
    ...
    // Also delete any large data if stored separately in localStorage
    if (draftReview.large_data_id) {
      localStorage.removeItem(draftReview.large_data_id);
    }
    
    // Remove from tracking
    await removeDraftId(business.id);
  ...
  const migrateLocalDraftToDatabase = async () => {
    ...
    const localDraft = await getLocalDraft();
 ...
 // Separate function for auto scroll
  ...
  setTimeout(() => {
    ...
    if (element) {
      element.scrollIntoView({ 
        behavior: 'smooth', 
        block: 'center' // ya 'start'/'end' as per preference
      });
    }
  }, 300); // Thoda delay taki form properly render ho jaye
};

// handleAddReview function mein
const handleAddReview = () => {
  ...
  const saveDraftReview = async () => {
    ...
    try {
      ...
      const savedDraft = await saveToLocalStorage(draftData);
      if (savedDraft) {
        setDraftReview(savedDraft);
            ...
            <div>
              ...
              {activeTab === 'reviews' ? (
                <>
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-sm text-amber-600">
                              ...
                              {user ? ' Saved to your account' : ' Saved in cookies'} • 
                  ...
                  {showReviewForm && (
                    <div className="mb-8 p-6 border border-green-200 rounded-xl bg-green-50" id="review-section">
                      ...
                      <form onSubmit={handleSubmitReview} className="space-y-6">
                        ...
                        <div>
                          ...
                          {!user && (
                            <p className="text-xs text-gray-500 mt-2">
                              ✓ Proof is saving as draft in cookies. After signup, it will be uploaded to your account.
                            </p>
                          )}
                        </div>
                        ...
                        {!user && (
                          <p className="text-sm text-gray-600 text-center">
                            ✓ Draft saved in browser cookies (7 days expiry). Sign up to save it permanently to your account.
                          </p>
                        )}
                      </form>
                    </div>
            ...
            <div className="border border-gray-200 rounded-xl p-6">
              ...
              <div className="space-y-3">
                ...
                <div className="flex items-start gap-2">
                  ...
                  <p className="text-sm text-gray-600">Save draft in cookies, publish after verification</p>
                </div>
              </div>
            </div>
